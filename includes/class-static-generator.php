...
class BSP_Static_Generator {
	...
	private function optimize_html( $html, $post_id ) {
		...
		// Add SEO protection to prevent duplicate content issues.
		$html = $this->add_seo_protection( $html, $post_id );
	 ...
	 * @param  string $html    HTML content to optimize.
	 * @param  int    $post_id Post ID for context.
	 * @return string HTML content with SEO protection.
	 */
	private function add_seo_protection( $html, $post_id ) {
		// Get the canonical URL (original dynamic URL).
		...
		// Add canonical URL to point to the original dynamic page.
		$seo_meta[] = sprintf( '<link rel="canonical" href="%s">', esc_url( $canonical_url ) );

		// Add robots meta tag to prevent indexing of static version.
		// This ensures only the original dynamic URL gets indexed.
		$robots_content = apply_filters( 'bsp_static_robots_meta', 'noindex, nofollow', $post_id );
		$seo_meta[] = sprintf( '<meta name="robots" content="%s">', esc_attr( $robots_content ) );

		// Add X-Robots-Tag equivalent as meta for extra safety.
		$seo_meta[] = '<meta name="googlebot" content="noindex, nofollow">';
		$seo_meta[] = '<meta name="bingbot" content="noindex, nofollow">';

		// Add meta tag to identify this as a static version.
		$seo_meta[] = '<meta name="bsp-static-version" content="true">';
		$seo_meta[] = sprintf( '<meta name="bsp-original-url" content="%s">', esc_url( $canonical_url ) );

		// Combine all SEO meta tags.
		$seo_meta_string = "\n" . implode( "\n", $seo_meta ) . "\n";

		// Insert SEO meta tags in the head section.
		$html = str_replace( '</head>', $seo_meta_string . '</head>', $html );

		// Add structured data markup to indicate this is not the canonical version.
